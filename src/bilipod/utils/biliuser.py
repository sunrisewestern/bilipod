...
async def get_pod_info(
    ...
    if uid:
        ...
        raise ValueError("One of uid, sid, or media_id must be provided.")
...
async def get_favorite_list_info(
    ...
    return {
        "media_id": media_id,
        "title": info.get("title", f"Favorite List {media_id}"),
        "description": info.get("intro", ""),
        "cover_art": info.get("cover")
        or (episodes_info[0]["image"] if episodes_info else ""),
        "author": upper.get("name", ""),
        "link": (
            f"https://space.bilibili.com/{upper['mid']}/favlist?fid={media_id}"
            if upper.get("mid")
            else f"https://space.bilibili.com/favlist?fid={media_id}"
        ),
